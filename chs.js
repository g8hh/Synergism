...
var cnItems = {
    ...
    '[5x10] What doesn\'t this boost? +0.01% Accelerators, Multipliers, Accelerator Boosts, +0.02% Obtainium, +0.02% Offerings, +0.1 Max Rune Levels, +1 Effective ELO, +0.001 Talisman bonuses per level.': '[5x10]它无所不包。每级使加速器、加倍器、加速器加成增加0.01%，难得素和祭品获取数量增加0.02%，符文等级上限增加0.1，有效强度分增加1，护身符的每级符文等级加成增加0.001。',
    //惊奇方盒结束
...
//需排除的，正则匹配
var cnExcludeWhole = [
    /^x?\d+(\.\d+)?[A-Za-rt-z%]{0,2}(\s.C)?\s*$/, //12.34K,23.4 °C
    /^x?\d+(\.\d+)?(e[+\-]?\d+)?\s*$/, //12.34e+4
    /^\s*$/, //纯空格
    /^ (.+)M $/, //纯空格
    /^: (.+) \[(.+)\]$/, //纯空格
    /^\d+(\.\d+)?[A-Za-rt-z]{0,2}.?\(?([+\-]?(\d+(\.\d+)?[A-Za-rt-z]{0,2})?)?$/, //12.34M (+34.34K
    /^(\d+(\.\d+)?[A-Za-rt-z]{0,2}\/s)?.?\(?([+\-]?\d+(\.\d+)?[A-Za-rt-z]{0,2})?\/s\stot$/, //2.74M/s (112.4K/s tot
