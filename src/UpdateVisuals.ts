    ...
    for (const key of keys) {
        ...
        if (shopItem.type === shopUpgradeTypes.UPGRADE) {
            ...
            if (!player.shopBuyMaxToggle) {
                ...
                DOMCacheGetOrSet(`${key}Button`).textContent = player.shopUpgrades[key] >= shopItem.maxLevel ? 'Maxed!' : '使用' + format(metaData.cost) + '夸克升级' + format(metaData.levelCanBuy - player.shopUpgrades[key], 0, true) + '级';
            }
