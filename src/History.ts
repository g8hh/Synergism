...
const resetHistoryRenderRow = (
    ...
    if (data.kind === 'antsacrifice') {
        ...
        extra.push(
            `<span title="蚂蚁倍率：${format(oldMulti, 3, false)}--&gt;${format(newMulti, 3, false)}"><img src="Pictures/${IconSets[player.iconSet][0]}/Multiplier.png" alt="Ant Multiplier">+${format(diff, 3, false)}</span>`,
            `<span title="每秒增加${formatDecimalSource(data.crumbsPerSecond)}银河碎屑"><img src="Pictures/${IconSets[player.iconSet][0]}/TinyCrumbs.png" alt="Crumbs">${extractStringExponent(formatDecimalSource(data.crumbs))}</span>`,
            `<span title="基础值为${format(data.baseELO)}"><img src="Pictures/${IconSets[player.iconSet][0]}/TinyELO.png" alt="ELO">${format(data.effectiveELO)}</span>`
        );
    } else if (data.kind === 'ascend') {
        extra.push(
            `<img alt="C10" src="Pictures/${IconSets[player.iconSet][0]}/TinyChallenge10.png" title="挑战10完成次数">${data.c10Completions}`
        );
