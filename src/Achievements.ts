...
export const areward = (i: number): string => {
    ...
    const descs: {[key: number]: string} = {
        ...
        280: '进入奇点后直接完成一次挑战8，并获得一只工蚁。(隐藏效果：进入奇点后直接完成成就176至成就182，直接获得更好的自动研究效果) ---- (15次奇点的隐藏效果：进入奇点后直接完成五次挑战8，获得一只蚁后，并获得2.22e2222粒子，可以自动购买祝福和魂灵，挑战13在计算挑战的完成次数时，视为减少4次完成次数) ---- (20次奇点的隐藏效果：进入奇点后直接完成一次挑战9，获得1e100银河碎屑，并获得虔蚁护身符，夸克获取数量永久增加5%，获得商店的EX额外祭品、AUTO自动献祭祭品、EX额外难得素、AUTO难得素自动研究、蚂蚁速度、昂贵物品至满级(同时变为无法重置)，挑战13在计算挑战的完成次数时，视为减少1次完成次数) ---- (25次奇点的隐藏效果：进入奇点后直接获得一只神使蚁) ---- (30次奇点的隐藏效果：可以自动购买符文6 - 无限晋升)'
    }
