...
export const areward = (i: number): string => {
    ...
    const descs: {[key: number]: string} = {
        ...
        280: '进入奇点后直接完成一次挑战8，并获得一只工蚁。(隐藏效果：进入奇点后直接获得自动购买粒子升级) ---- (15次奇点的隐藏效果：进入奇点后自动符文可以自动献祭所有祭品，挑战13在计算挑战的完成次数时，视为减少4次完成次数) ---- (20次奇点的隐藏效果：进入奇点后直接获得1e100银河碎屑，并获得虔蚁护身符，挑战13在计算挑战的完成次数时，视为减少1次完成次数) ---- (30次奇点的隐藏效果：可以自动加强和升阶护身符，可以自动购买死神蚁)'
    }
