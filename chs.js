...
var cnExcludeWhole = [
    ...
    /^\s*$/, //纯空格
    /^\s*[A-Za-z]\s*$/, //带单字母的纯空格
    /^速度符文奖励：加速器数量\+(.+)，加速器数量再\+(.+)，加速器加成数量\+(.+)$/, //控制台信息处理
    /^重叠符文奖励：加倍器数量\+(.+)，加倍器数量再\+(.+)%，税收增长减缓(.+)%。$/, //控制台信息处理
    /^棱柱符文奖励：水晶产量变为(.+)倍，水晶升级免费\+(.+)级。$/, //控制台信息处理
    /^节俭符文奖励：所有建筑成本增长减缓(.+)%。祭品回收概率：\+(.+)%。税收增长减缓(.+)%。$/, //控制台信息处理
    /^卓越智慧符文奖励：难得素获取数量变为(.+)倍。蚂蚁速度变为(.+)倍。基础祭品：\+(.+)$/, //控制台信息处理
];
