...
export class HepteractCraft {
    ...
    craft = async (max = false): Promise<HepteractCraft | void> => {
        ...
        if (!max) {
            const craftingPrompt = await Prompt(`您想要锻造多少？\n您至多可以锻造${format(amountToCraft, 0, true)}个(上限的${(Math.floor(amountToCraft / this.CAP * 10000) / 100)}%)。`);
        ...
        if (max && player.toggles[35]) {
            const craftYesPlz = await Confirm(`这将尽可能进行锻造。\n您至多可以锻造${format(amountToCraft, 0, true)}个(上限的${(Math.floor(amountToCraft / this.CAP * 10000) / 100)}%)。您确定要这么做吗？`);
            if (!craftYesPlz) {
                return Alert('Okay, maybe next time.');
            }
        }
...
export const hepteractDescriptions = (type: hepteractTypes) => {
    ...
    switch (type){
        ...
        case 'quark':
            ...
            costText.textContent = '锻造一个将消耗' + format(player.hepteractCrafts.quark.HEPTERACT_CONVERSION * craftCostMulti, 0, true) + '七阶立方及100夸克。'
            break;
        case 'challenge':
...
export const tradeHepteractToOverfluxOrb = async (buyMax?:boolean) => {
    ...
    if (buyMax) {
        if (player.toggles[35]) {
            const craftYesPlz = await Confirm(`这将尽可能融合超通量珠\n您至多可以锻造${format(maxBuy, 0, true)}个。您确定要这么做吗？`);
            if (!craftYesPlz) {
                return Alert('Okay, maybe next time.');
            }
        }
        ...
        const hepteractInput = await Prompt(`您要购买多少超通量珠？\n现有的七阶立方最多可以购买${format(maxBuy, 0, true)}。`);
