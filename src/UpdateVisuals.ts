...
export const visualUpdateBuildings = () => {
    ...
    if (G['buildingSubTab'] === 'particle') {
        ...
        DOMCacheGetOrSet('reincarnationshardinfo').textContent = '您有' + format(player.reincarnationShards, 2) + '原子，建筑能量因此增加' + format(G['buildingPower'], 4) + '。金币产量倍率变为' + format(G['reincarnationMultiplier']) + '倍'
...
export const visualUpdateAnts = () => {
    ...
    DOMCacheGetOrSet('autoAntSacrifice').textContent = `当时间至少经过${player.autoAntSacTimer}秒(${mode})后献祭蚂蚁，目前时间为：${format(timer, 2)}秒`
        ...
        if (shopItem.type === shopUpgradeTypes.CONSUMABLE) {
            ...
            switch (player.shopBuyMaxToggle) {
                case false:
                    ...
                    break;
                case 'TEN':
                    el.textContent = `花费${format(getShopCosts(key)*Math.min(10,maxBuyablePotions),0,true)}夸克购买${Math.min(10,maxBuyablePotions)}瓶`;
                    break;
                default:
                    el.textContent = `花费${format(getShopCosts(key)*maxBuyablePotions)}夸克购买${maxBuyablePotions}瓶`;
            }
        }
        ...
        if (shopItem.type === shopUpgradeTypes.UPGRADE) {
            ...
            switch (player.shopBuyMaxToggle) {
                ...
                case 'TEN':
                    ...
                    DOMCacheGetOrSet(`${key}Button`).textContent = player.shopUpgrades[key] >= shopItem.maxLevel ? 'Maxed!' : `使用${format(buyData.cost)}夸克升${format(buyData.levelCanBuy - player.shopUpgrades[key], 0, true)}级`;
                    break;
                default:
