...
export const loadStatisticsMiscellaneous = () => {
    ...
    DOMCacheGetOrSet('sMisc2').textContent = format(1000 * player.fastestprestige) + '毫秒'
    ...
    DOMCacheGetOrSet('sMisc6').textContent = format(1000 * player.fastesttranscend) + '毫秒'
    DOMCacheGetOrSet('sMisc7').textContent = format(player.reincarnationCount, 0, true)
    DOMCacheGetOrSet('sMisc8').textContent = format(1000 * player.fastestreincarnate) + '毫秒'
...
export const loadAddCodeModifiersAndEffects = () => {
    ...
    for (let i = 0; i < intervalStats.list.length; i++) {
        ...
        if (i == 0) {
            DOMCacheGetOrSet(`s+time${i + 1}`).textContent = `${format(intervalStats.list[i], intervalMap[i + 1].acc, true)}秒`;
        } else {
            DOMCacheGetOrSet(`s+time${i + 1}`).textContent = `x${format(intervalStats.list[i], intervalMap[i + 1].acc, true)}`;
        }
    }

    DOMCacheGetOrSet('s+timeT').textContent = `${format(intervalStats.time / 1000, 1)}秒`;
    if (availableCount != capacityStats.total) {
        DOMCacheGetOrSet('s+next').textContent = `${format(timeToNext, 1)}秒后增加1次`;  // is already in sec.
    } else {
    ...
    DOMCacheGetOrSet('s+eff2').textContent = `+${format(addEffectStats.ascensionTimer, 2)}秒`;

    DOMCacheGetOrSet('stat+eff3').childNodes[0].textContent = 'PL-AT Γ - bonus GQ export time: ';
    DOMCacheGetOrSet('s+eff3').textContent = `+${format(addEffectStats.gqTimer, 2)}秒`; // does it need a / 1000?

    DOMCacheGetOrSet('stat+eff4').childNodes[0].textContent = 'PL-AT _ - bonus octeract time: ';
    DOMCacheGetOrSet('s+eff4').textContent = `+${format(addEffectStats.octeractTime, 2)}秒`; // does it need a / 1000?
...
export const gameStages = (): Stage[] => {
    const stages: Stage[] = [
        {stage: 0, tier: 1, name: '初始', unlocked: true, reset: true},
        {stage: 1, tier: 1, name: '初始至转生', unlocked: player.unlocks.prestige === true, reset: player.unlocks.prestige},
        {stage: 2, tier: 2, name: '转生至超越', unlocked: player.unlocks.transcend === true, reset: player.unlocks.transcend},
        {stage: 3, tier: 3, name: '超越至转世', unlocked: player.unlocks.reincarnate === true, reset: player.unlocks.reincarnate},
        {stage: 4, tier: 4, name: '转世至蚂蚁', unlocked: player.firstOwnedAnts !== 0, reset: player.unlocks.reincarnate},
        {stage: 5, tier: 4, name: '蚂蚁至蚂蚁献祭', unlocked: player.achievements[173] === 1, reset: player.unlocks.reincarnate},
        {stage: 6, tier: 4, name: '蚂蚁献祭至飞升', unlocked: player.achievements[183] === 1, reset: player.unlocks.reincarnate},
        {stage: 7, tier: 5, name: '飞升至挑战10', unlocked: player.ascensionCount > 1, reset: player.achievements[183] === 1},
        {stage: 8, tier: 5, name: '挑战10至挑战11', unlocked: player.achievements[197] === 1, reset: player.achievements[183] === 1},
        {stage: 9, tier: 5, name: '挑战11至挑战12', unlocked: player.achievements[204] === 1, reset: player.achievements[183] === 1},
        {stage: 10, tier: 5, name: '挑战12至挑战13', unlocked: player.achievements[211] === 1, reset: player.achievements[183] === 1},
        {stage: 11, tier: 5, name: '挑战13至挑战14', unlocked: player.achievements[218] === 1, reset: player.achievements[183] === 1},
        {stage: 12, tier: 5, name: '挑战14至方盒升级5x10购买最大', unlocked: player.cubeUpgrades[50] >= 100000, reset: player.achievements[183] === 1},
        {stage: 13, tier: 5, name: '方盒升级5x10购买最大至阿尔法升级', unlocked: player.platonicUpgrades[5] > 0, reset: player.achievements[183] === 1},
        {stage: 14, tier: 5, name: '阿尔法升级至P2x1x10升级', unlocked: player.platonicUpgrades[6] >= 10, reset: player.achievements[183] === 1},
        {stage: 15, tier: 5, name: 'P2x1x10升级至P3x1升级', unlocked: player.platonicUpgrades[11] > 0, reset: player.achievements[183] === 1},
        {stage: 16, tier: 5, name: 'P3x1升级至贝塔升级', unlocked: player.platonicUpgrades[10] > 0, reset: player.achievements[183] === 1},
        {stage: 17, tier: 5, name: '贝塔升级至1e15指数', unlocked: player.challenge15Exponent >= 1e15, reset: player.achievements[183] === 1},
        {stage: 18, tier: 5, name: '1e15指数至欧米伽升级', unlocked: player.platonicUpgrades[15] > 0, reset: player.achievements[183] === 1},
        {stage: 19, tier: 5, name: '欧米伽升级至奇点', unlocked: player.singularityCount > 0 && player.runelevels[6] > 0, reset: player.achievements[183] === 1},
        {stage: 20, tier: 6, name: '奇点至商店清仓大甩卖', unlocked: player.singularityUpgrades.wowPass2.level > 0, reset: player.highestSingularityCount > 0},
        {stage: 21, tier: 6, name: '商店清仓大甩卖至一心', unlocked: player.singularityUpgrades.oneMind.level > 0, reset: player.highestSingularityCount > 0},
        {stage: 22, tier: 6, name: '一心至蓝莓碎片', unlocked: player.singularityUpgrades.offeringAutomatic.level > 0, reset: player.highestSingularityCount > 0},
        {stage: 23, tier: 6, name: '蓝莓碎片至终极妙笔', unlocked: player.singularityUpgrades.ultimatePen.level > 0, reset: player.highestSingularityCount > 0},
        {stage: 24, tier: 6, name: '终极妙笔', unlocked: false, reset: player.highestSingularityCount > 0}
    ];
    return stages;
}
